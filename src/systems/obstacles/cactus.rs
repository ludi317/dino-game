...
pub(crate) fn generate_cactus(width: u32, height: u32, rng: &mut GlobalEntropy<WyRand>) -> Image {
    ...
    let right_top_square_x =
        right_tall_rect_x + (right_tall_rect_width - right_top_square_size) / 2;
    ...
    for _ in 0..2 {
        let square_x =
            (trunk_x + trunk_width) + (rng.next_u32() as u32 % (arm_width - square_size));
    ...
    for _ in 0..1 {
        let square_x =
            right_tall_rect_x + (rng.next_u32() as u32 % (right_tall_rect_width - square_size));
        let square_y =
            right_tall_rect_y + (rng.next_u32() as u32 % (right_tall_rect_height - square_size));
